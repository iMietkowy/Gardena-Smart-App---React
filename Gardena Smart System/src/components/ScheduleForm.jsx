...
const ScheduleForm = ({ devices, onAddSchedule }) => {
    ...
    const actionOptions = useMemo(() => {
        ...
        if (selectedDevice.type === 'SMART_WATERING_COMPUTER') return [{ value: 'startWatering', label: 'Rozpocznij podlewanie' }];
        return [];
    }, [selectedDevice]);

    const hourOptions = useMemo(() => Array.from({ length: 24 }, (_, i) => ({ value: String(i).padStart(2, '0'), label: String(i).padStart(2, '0') })), []);
    const minuteOptions = useMemo(() => Array.from({ length: 60 }, (_, i) => ({ value: String(i).padStart(2, '0'), label: String(i).padStart(2, '0') })), []);
    ...
    return (
        <div className="schedule-form-container">
            ...
            <form onSubmit={handleSubmit} className="schedule-form">
                ...
                <CustomSelect value={selectedDeviceId} onChange={setSelectedDeviceId} options={deviceOptions} placeholder="-- Wybierz urządzenie --" />
                
                {selectedDevice?.type === 'SMART_WATERING_COMPUTER' && (
                    <><label>Zawór</label><CustomSelect value={selectedValveId} onChange={setSelectedValveId} options={valveOptions} placeholder="-- Wybierz zawór --" /></>
                )}
                
                {selectedDevice && (
                    <><label>Akcja</label><CustomSelect value={action} onChange={setAction} options={actionOptions} placeholder="-- Wybierz akcję --" /></>
                )}
                
                <div className="time-range-selector">
                    <div className="time-picker-group"><label>Od:</label><CustomSelect value={startHour} onChange={setStartHour} options={hourOptions} /><span>:</span><CustomSelect value={startMinute} onChange={setStartMinute} options={minuteOptions} /></div>
                    <div className="time-picker-group"><label>Do:</label><CustomSelect value={endHour} onChange={setEndHour} options={hourOptions} /><span>:</span><CustomSelect value={endMinute} onChange={setEndMinute} options={minuteOptions} /></div>
                </div>
                
                <label>Dni tygodnia</label>
                <div className="btn-group">
                    {daysOfWeekUIOrder.map(day => (
                        <button type="button" key={day.short} onClick={() => handleDayChange(day.short)} className={`btn-toggle ${days.includes(day.short) ? 'active' : ''}`}>{day.short}</button>
                    ))}
                </div>
                
                <button type="submit" className="btn btn--primary btn--rounded">Dodaj do harmonogramu</button>
            </form>
        </div>
    );
};
