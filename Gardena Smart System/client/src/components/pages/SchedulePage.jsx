...
const SchedulePage = () => {
    ...
    const handleMassAction = (actionType) => {
        ...
        const messages = {
            delete: selectedDeviceForSchedule === 'all' ? 'Czy na pewno chcesz usunąć WSZYSTKIE harmonogramy?' : `Czy na pewno chcesz usunąć wszystkie harmonogramy dla "${deviceName}"?`,
            disable: selectedDeviceForSchedule === 'all' ? 'Czy na pewno chcesz wstrzymać WSZYSTKIE harmonogramy?' : `Czy na pewno chcesz wstrzymać harmonogramy dla "${deviceName}"?`,
            enable: selectedDeviceForSchedule === 'all' ? 'Czy na pewno chcesz wznowić WSZYSTKIE harmonogramy?' : `Czy na pewno chcesz wznowić harmonogramy dla "${deviceName}"?`,
        };
    ...
    const hasSchedulesForFilter = schedules.some(s => selectedDeviceForSchedule === 'all' || s.deviceId === selectedDeviceForSchedule);
    const hasEnabledSchedules = schedules.some(s => (selectedDeviceForSchedule === 'all' || s.deviceId === selectedDeviceForSchedule) && s.enabled);
    const hasDisabledSchedules = schedules.some(s => (selectedDeviceForSchedule === 'all' || s.deviceId === selectedDeviceForSchedule) && !s.enabled);
            ...
            <div className='schedule-container'>
                ...
                <div className='schedule-list-container'>
                    ...
                    <div className='schedule-filter-device'>
                        ...
                        <CustomSelect value={selectedDeviceForSchedule} onChange={setSelectedDeviceForSchedule} options={filterDeviceOptions} />
                        <div className='schedule-actions-group'>
                            {hasEnabledSchedules && <button onClick={() => handleMassAction('disable')} className='btn btn--secondary btn--rounded'>Wstrzymaj</button>}
                            {hasDisabledSchedules && <button onClick={() => handleMassAction('enable')} className='btn btn--primary btn--rounded'>Wznów</button>}
                            {hasSchedulesForFilter && <button onClick={() => handleMassAction('delete')} className='btn btn--danger btn--rounded'>Wyczyść</button>}
                        </div>
                    </div>
